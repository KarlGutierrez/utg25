...
class Game:
    ...
    def init(self):
        ...
        for i in range(town_count):
            ...
            self._town_coords_set.add((town_x, town_y))
    ...
    def find_shortest_path(self, from_town_id: int, to_town_id: int) -> tuple[List[Coord], int, int, int]:
        """
        OPTIMIZED Dijkstra:
        Uses 'came_from' parent pointers instead of storing full path lists in the Priority Queue.
        Eliminates timeouts while keeping exact same logic.
        """
        ...
        # PQ stores: (cost, unique_counter, y, x)
        # Removed 'path' list from here to save massive memory/CPU
        ...
        counter = 0 # To preserve FIFO on ties
        ...
        while pq:
            ...
            # Lazy Deletion (if we found a faster way to here already)
        ...
        # Calculate Points (Same logic as before)
    ...
    def place_tracks_smartly(self, desired_paths: List):
            ...
            for from_town, to_town, path, path_cost, my_points, foe_points in desired_paths:
                ...
                for coord in path:
                    ...
                    if coord_tuple not in self._town_coords_set:
                ...
                for coord in path:
                    ...
                    # Skip towns
                    if coord_tuple in self._town_coords_set:
                        continue
            ...
            # Phase 3.5: Build in safe regions (Towns)
            # Interpreted "regions with first" as Regions with Towns (Safe Zones)
            if paint_used < paint_available:
                print(f"Phase 3.5: Scanning safe town regions with {paint_available - paint_used} paint", file=sys.stderr)
                safe_tiles = []
                for region in self.region_by_id.values():
                    if region.has_town and not region.inked:
                        for coord in region.coords:
                            coord_tuple = (coord.x, coord.y)
                            
                            # Skip if already built or is the town itself
                            if coord_tuple in built_coords or coord_tuple in self._town_coords_set:
                                continue
                            
                            tile = tiles[coord.y][coord.x]
                            if tile.tracks_owner == -1: # Empty
                                cost = type_costs[tile.type]
                                safe_tiles.append((coord, cost))
                
                safe_tiles.sort(key=lambda x: x[1]) # Cheapest first
                
                for coord, track_cost in safe_tiles:
                    if paint_used + track_cost <= paint_available:
                        actions.append(f"PLACE_TRACKS {coord.x} {coord.y}")
                        paint_used += track_cost
                        built_coords.add((coord.x, coord.y))
                        print(f"Phase 3.5: Safe build at ({coord.x},{coord.y})", file=sys.stderr)
                    
                    if paint_used == paint_available:
                        break

            # Phase 4: LAST RESORT - scan entire map for ANY buildable tile
            # Prioritizing Center of map
            if paint_used < paint_available:
                ...
                for y in range(height):
                    for x in range(width):
                        ...
                        if tile.tracks_owner == -1 and not tile.inked:
                            ...
                            # Calculate distance to center (Manhattan)
                ...
                # Sort: 1. Cost (Ascending), 2. Distance to Center (Ascending)
