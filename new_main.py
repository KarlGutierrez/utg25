...
class Game:
    ...
    def get_disruption_target(self) -> int | None:
        ...
        for region_id, region in self.region_by_id.items():
            ...
            # Find all active connections that pass through this region
            ...
            # If no active connections would be affected, skip this region
            if not affected_connections:
                continue
            
            # For each affected connection, count tracks across the entire grid
            my_total_tracks_in_connections = 0
            foe_total_tracks_in_connections = 0
            
            for from_id, to_id in affected_connections:
                my_tracks_in_this_connection = 0
                foe_tracks_in_this_connection = 0
                
                # Scan entire grid for tiles that are part of this connection
                for y in range(self.grid.height):
                    for x in range(self.grid.width):
                        tile = self.grid.tiles[y][x]
                        
                        # Check if this tile is part of the current connection
                        is_part_of_connection = any(
                            conn.from_id == from_id and conn.to_id == to_id 
                            for conn in tile.part_of_active_connections
                        )
                        
                        if is_part_of_connection:
                            if tile.tracks_owner == self.my_id:
                                my_tracks_in_this_connection += 1
                            elif tile.tracks_owner == foe_id:
                                foe_tracks_in_this_connection += 1
                
                my_total_tracks_in_connections += my_tracks_in_this_connection
                foe_total_tracks_in_connections += foe_tracks_in_this_connection
            
            # Skip if we have more tracks in the affected connections than the foe
            if my_total_tracks_in_connections > foe_total_tracks_in_connections:
                print(f"Skipping region {region_id}: our tracks in connections ({my_total_tracks_in_connections}) > foe tracks ({foe_total_tracks_in_connections})", file=sys.stderr)
                continue
            
            # Count tracks just in this region for value calculation
            ...
            # Skip if we have our own tracks in this region
            if my_tracks_in_region > 0:
                continue
            
            # Skip if no enemy tracks in this region
        ...
        if disruption_candidates:
            ...
            for i, cand in enumerate(disruption_candidates[:3]):
                print(f"  {i+1}. Region {cand['region_id']}: value={cand['value']}, affected_conn={cand['affected_connections']}, foe_total={cand['foe_total_tracks']}, my_total={cand['my_total_tracks']}, instability={cand['instability']}/3", file=sys.stderr)
