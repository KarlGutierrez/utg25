...
class Game:
    ...
    def get_disruption_target(self) -> int | None:
        """Choose the best region to disrupt, or ``None`` if none is worth it.

        **IMPROVED STRATEGY:**

        A candidate region must:
        - not already be inked and not contain a town,
        - contain at least one opponent track,
        - participate in at least one active connection.

        **Key improvements:**
        1. **Allows disrupting regions where we have some tracks** if the benefit
        outweighs the cost (i.e., we destroy more enemy tracks than our own).
        2. **Prioritizes high-value enemy connections** by considering the total
        point value of connections (foe's track count in each connection).
        3. **Weighs connections by their completion status** - connections where
        the enemy has many tracks are more valuable to disrupt.

        Each candidate is scored based on:
        - Net benefit: (foe tracks - my tracks) in the region (×5 weight)
        - High-value enemy connections: sum of foe track counts (×4 weight)
        - Number of distinct connections affected (×3 weight)
        - Proximity to inking: instability level (×2 weight)

        Returns
        -------
        Optional[int]
            The ``region_id`` to target with a ``DISRUPT`` action, or ``None`` if
            disrupting any region appears unprofitable.
        """
        foe_id = 1 - self.my_id
        ...
        for region_id, region in self.region_by_id.items():
            ...
            # Quick filters - must have connections and at least one foe track
            if not affected_connections or foe_tracks_in_region == 0:
                continue

            # NEW: Allow disrupting if we destroy more enemy tracks than our own
            net_tracks_destroyed = foe_tracks_in_region - my_tracks_in_region
            if net_tracks_destroyed <= 0:
                # Skip if we'd destroy more of our own tracks or break even
                continue

            # Calculate connection values and totals
            my_total_tracks_in_connections = 0
            foe_total_tracks_in_connections = 0
            high_value_enemy_connections = 0  # Sum of foe track counts in affected connections

            for conn_key in affected_connections:
                if conn_key in connection_tracks:
                    my_count = connection_tracks[conn_key]['my']
                    foe_count = connection_tracks[conn_key]['foe']
                    my_total_tracks_in_connections += my_count
                    foe_total_tracks_in_connections += foe_count
                    # NEW: Track high-value enemy connections
                    # More foe tracks = higher value connection to disrupt
                    high_value_enemy_connections += foe_count

            # NEW: More lenient filter - allow if benefit exists
            # Skip only if we're massively ahead in affected connections
            if my_total_tracks_in_connections > foe_total_tracks_in_connections * 1.5:
                continue

            # Calculate disruption value with improved weighting
            ...
            # NEW FORMULA with better weights:
            value = (
                net_tracks_destroyed * 5 +              # Direct net benefit (highest weight)
                high_value_enemy_connections * 4 +      # NEW: Prioritize disrupting valuable enemy connections
                len(affected_connections) * 3 +         # Breadth of disruption
                net_connection_benefit * 3 +            # Overall connection advantage
                (3 - disruptions_to_ink) * 2            # Ease of inking (lower weight)
            )
            ...
            disruption_candidates.append({
                ...
                'my_tracks_in_region': my_tracks_in_region,
                'net_tracks_destroyed': net_tracks_destroyed,
                'affected_connections': len(affected_connections),
                'high_value_connections': high_value_enemy_connections,
        ...
        if disruption_candidates:
            ...
            print(f"Disrupt: R{best['region_id']} v={best['value']} net={best['net_tracks_destroyed']} "
                f"hv={best['high_value_connections']}", file=sys.stderr)
